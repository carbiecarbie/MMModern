...
XeenObjectVisualResolver resolver() {
	std::vector<std::uint8_t> b(1452);
	for(int id:{111,113}) for(int d=0;d<4;++d) {
		b[id*12+d]=d; b[id*12+4+d]=d%2; b[id*12+8+d]=d;
	}
...
void projection() {
	...
	for(int row=0;row<2;++row) for(int direction=0;direction<4;++direction) {
		...
		const auto all=XeenOutdoorScene().build(world,{23,8,8,static_cast<XeenDirection>(direction)},&r);
		...
		for(const auto &c:commands) {
			...
			check(v.frame==static_cast<unsigned>(direction) && v.horizontalFlip==(direction%2!=0) && o.horizontalFlip==v.horizontalFlip,"authoritative 16A direction");
		}
	}
}
...
int main(){try{projection();stateAndPrecedence();boundary();std::cout<<"Outdoor object commands passed\n";return 0;}
