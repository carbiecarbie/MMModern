...
int main(int argc,char **argv) {
 try {
  ...
  {Harness h(game);auto s=regional(h);s.show=[&](const auto &,const auto &handler,const auto &,const auto &,const auto &){handler.framePresented();source=XeenSaveState::capture(h.signature,*h.party,*h.camera,*h.flags,*h.world);return true;};
  ...
  for(auto cell:{std::pair<int,int>{0,1},{4,5},{5,9},{5,13},{7,7},{8,2},{8,10},{9,11},{10,13},{12,12}})for(unsigned d=0;d<4;++d) {
   ...
   s.show=[&](const auto &,const auto &handler,const auto &,const auto &idle,const auto &){
    shown=true;handler.framePresented();const auto before=bytes(h);
    ...
    h.flow->refresh(true);handler.framePresented();
    for(unsigned n=0;!h.flow->canSave() && n<100;++n){h.now+=100;handler.beginCycle(++h.cycle);idle();handler.framePresented();}
  ...
  for(unsigned fault=0;fault<5;++fault) {
   ...
   s.show=[&](const auto &,const auto &handler,const auto &,const auto &,const auto &){
    shown=true;handler.framePresented();const auto before=bytes(h);
    if(fault<=1) {
     handler.beginCycle(++h.cycle);handler.withDisplayedInput(InteractionAction{},*handler.displayedInput());handler.framePresented();
    ...
    bool failed=false;try{h.flow->refresh(true);handler.framePresented();}catch(const std::exception &){failed=true;}
