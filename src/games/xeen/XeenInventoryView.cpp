...
std::vector<XeenInventoryLine> xeenInventoryLayout(const XeenFontFormat &font,
	...
	if (character) {
		...
		XeenCharacterRules::validateForUse(c, {party.encounterContext ? std::uint32_t(party.encounterContext->year) : kCloudsInitialYear});
		...
		line(10,310,17,"HP " + std::to_string(c.currentHp) + " / " + std::to_string(XeenCharacterRules::maxHp(c,{party.encounterContext ? std::uint32_t(party.encounterContext->year) : kCloudsInitialYear})));
		line(10,310,26,"SP " + std::to_string(c.currentSp) + " / " + std::to_string(XeenCharacterRules::maxSp(c,{party.encounterContext ? std::uint32_t(party.encounterContext->year) : kCloudsInitialYear})));
